...
"""
import network
from station import MAX_RETRIES, NOAA_METADATA_MIN_BUDGET_SECONDS, Station
...
class TestGetStationBudgetBailout:
    ...
    def test_returns_after_one_attempt_when_budget_exhausted(self, monkeypatch):
        ...
        monkeypatch.setattr(network, 'request', fake_request)
        monkeypatch.setattr(network, '_budget_remaining', lambda: NOAA_METADATA_MIN_BUDGET_SECONDS - 1)
    ...
    def test_ample_budget_exhausts_max_retries(self, monkeypatch):
        ...
        monkeypatch.setattr(network, 'request', fake_request)
        monkeypatch.setattr(network, '_budget_remaining', lambda: NOAA_METADATA_MIN_BUDGET_SECONDS + 10)
